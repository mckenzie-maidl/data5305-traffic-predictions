...
ADF test statistic is higher than critical value so we cannot reject the null. Therefore, our data has unit root and it is not stationary. 
...
After we take the difference of the total sales, we can see that the data is stationary. This is further confirmed by the ADF test. Our ADF test statistics value is much smaller than the critical value. Therefore, we can confirm that the data is now stationary. 
...
top_5_bic
```
Based on the value, we choose ARMA(3,3), MA(1), and R(1) because these performed best when sorted by AIC, which is better than any models sorted by BIC method. The AIC value is also acceptable and BIC vales are not that different.

The AIC and BIC for ARMA(3,3) are about 942 and 968 respectively. For MA(1)	these are about 942 and 952, and for AR(1) they are 943 and 953.

### ARMA(3,3)
All ACF (except the first one) and PACF lags are within the significance lines, indicating they resemble white noise.

Given the p-value of over 0.9, we would fail to reject the null hypothesis of the Ljung-Box test, which states that the residuals are independently distributed (i.e., exhibit no autocorrelation). This is an indicator of a good model fit, as it implies that the residuals from the ARMA(3,3) model resemble white noise. 
```{r}
# create model
