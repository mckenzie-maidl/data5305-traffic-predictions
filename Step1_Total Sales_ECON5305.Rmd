...
```{r}
# upload data
total_sales <- read.csv('TOTALSA.csv')
head(total_sales)
```
...
pacf(ts1)
```
From intial ACF and PACF, we see that tour PACF values alternates between negative and positive and that there are spikes in the first two algs for ACF.
```{r}
...
top_5_bic
```
Based on the value, I chosoe ARMA (3,3) MA(1), AR(1) and ARMA /91,1) because these performed best when sorted by AIC ANd MIC.  which is better than any models sorted by BIC method. The AIC value is also acceptable and BIC vales are not that different. 
...
```
```{r}
# acf and pacf of model residuals for best model ARMA(3,3)
...
```
Given the p-value of 0.9745, you would fail to reject the null hypothesis of the Ljung-Box test, which states that the residuals are independently distributed (i.e., exhibit no autocorrelation). This is an indicator of a good model fit, as it implies that the residuals from your ARMA(3,3) model resemble white noise. 
...
```
MA(1,) also has residuals that resemeble whitw noise ( p value os 0.95), all ACF and PACF of residuals within the significance line. 
## MODEL ARMA(1)
```{r}
...
```
ARMA (2,2) also has 0.91 of p vlaue but it is less significance from other two models and also the PACF of the residuals has one significant vlaue. 
...
```
The AIC and BIC for ARMA(3,3) is 942.5077	968.3993 respectively. 
For MA(1)	is 	942.6554	952.3647	 and
for AR(1)	943.4648	953.1741	
