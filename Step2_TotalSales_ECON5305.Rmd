...
measure_performance <- function(actuals, predicted) {
  ...
  MPE <- summary(lm(error ~ 1))$coefficients[,3]
  
  # Informational Efficiency
  IE <- summary(lm(error ~ predicted))$coefficients[2,3]
...
generate_forecasts <- function(ts, model_order, h, perf_fun=measure_performance) {
  
  # train/test split
  train_index <- round(length(ts) * 0.9)
  ts_train <- ts[1:train_index]
  ts_test <- ts[(train_index+1):length(ts)]
  ...
  for (i in 1:ntest) {
    up <- ts[1:train_index-1+i] #window(ts, start=c(1960,1),end=c(2000,3+i))
    md <- Arima(up, model=m1)
    fcast1[i] <- forecast(md, h=h)$mean[h]
  }
  per1 <- perf_fun(ts_test, fcast1)
  per1 <- c('Fixed', per1)
...
# h = 1
arma33_1 <- generate_forecasts(ts1, order, 1)
arma33_1

# h = 2
arma33_2 <- generate_forecasts(ts1, order, 2)
arma33_2
```
...
# h = 1
ma1_1 <- generate_forecasts(ts1, order, 1)
ma1_1
...
# h = 1
ar1_1 <- generate_forecasts(ts1, order, 1)
ar1_1

# h = 2
ar1_2 <- generate_forecasts(ts1, order, 2)
ar1_2
```

## Combined Forecasts - NEED TO DO
